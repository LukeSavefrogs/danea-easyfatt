...
- Ora il programma gestisce anche la nomenclatura `Libero N` (oltre a `Extra N`) per le colonne personalizzabili nel foglio Excel dei clienti, impedendo il crash causato dal cambio di nomenclatura da parte di Easyfatt ([#258](https://github.com/LukeSavefrogs/danea-easyfatt/pull/258)).

## [v1.5.1] - 2026-03-12

### Fixed

- Corretto bug che causava la visualizzazione della selezione dell'indirizzo ad ogni geocodifica quando la configurazione `features.kml_generation.location_search_type` era impostata a `manual`
