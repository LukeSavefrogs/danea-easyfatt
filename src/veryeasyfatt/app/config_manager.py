...
def get_configuration(custom_config_file: (str | Path | None) = None) -> dict[str, Any]:
    ...
    if not default_config_file.exists():
        ...
        raise Exception(
            f"Impossibile trovare file di configurazione '{default_config_file}'. Solo la versione compilata può essere utilizzata senza configurazione."
        )
    ...
    if user_config_file.exists():
        ...
        logger.warning(
            f"File di configurazione utente '{user_config_file}' non trovato."
        )
